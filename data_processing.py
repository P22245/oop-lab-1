...
class Table:
    ...
    def filter(self,condition):
        ...
        return Table(self.name + '_fliterd', filter_data)
    ...
    def join(self,table_next_one,key):
        data_alredy_join = []
        for i in self.table:
            for j in table_next_one.table:
                if i[key] == j[key]:
                    join = i.copy()
                    join.update(j)
                    data_alredy_join.append(join)
        return Table(f"{self.name}_join_{table_next_one.name}", data_alredy_join)
